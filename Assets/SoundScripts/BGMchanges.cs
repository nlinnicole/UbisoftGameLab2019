...
public class BGMchanges : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Team1Room = gameObject.GetComponent<RoomGenerator>().team1CurrentRoom;
        Team2Room = gameObject.GetComponent<RoomGenerator>().team2CurrentRoom;

        if ((Team1Room == 0 || Team2Room == 0) && !roomsBGMstarted)
        {
            roomsBGMstarted = true;
            triggerRoomBGM();
        }

        if (roomsBGMstarted && (Team1Room > furthestRoom || Team2Room > furthestRoom))
        {
            furthestRoom += 1;
            triggerRoomBGM();
        }
    ...
    // finds the current voicing and 
    void setRandomVoice(int[] cues, int[][] voicings)
    {
        
            counter = (Time.time - bgmStarted - (3.2f)) % cues[cues.Length - 1];
        

        for (int i = 0; i < cues.Length - 1; i++)
        {

            if (counter > cues[i] && counter <= cues[i + 1])
            {
                voicing = voicings[i];
            }
        }
